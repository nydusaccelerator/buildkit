...
test () {
    ...
    for I in $(cat $IMAGE_LIST); do
        ...
        buildctl build --frontend dockerfile.v0 \
            ...
            --output type=image,name=$LOCAL_DICT,push=true,compression=nydus,nydus-fs-version=$NYDUS_FS_VERSION,oci-mediatypes=true,force-compression=true
        # build remote dict
        buildctl build --frontend dockerfile.v0 \
            --local context=./nydus-test/image \
            --local dockerfile=./nydus-test/top_images/ \
            --output type=image,name=$REMOTE_DICT,push=true,compression=nydus,nydus-fs-version=$NYDUS_FS_VERSION,oci-mediatypes=true,force-compression=true
        ...
        buildctl build --frontend dockerfile.v0 \
            --local context=./nydus-test/image \
            --local dockerfile=./nydus-test/top_images/ \
            --output type=image,name=$DEST_IMAGE,push=true,compression=nydus,nydus-fs-version=$NYDUS_FS_VERSION,force-compression=true,nydus-chunk-dict-image=$LOCAL_DICT
